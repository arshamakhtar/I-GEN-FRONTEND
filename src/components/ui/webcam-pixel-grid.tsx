  ...
  const render = useCallback(() => {
    ...
    for (let row = 0; row < gridRows; row++) {
      for (let col = 0; col < gridCols; col++) {
        ...
        const r = currentData[idx] ?? 0;
        const g = currentData[idx + 1] ?? 0;
        const b = currentData[idx + 2] ?? 0;
        ...
        let motion = 0;
        if (previousData) {
          const prevR = previousData[idx] ?? 0;
          const prevG = previousData[idx + 1] ?? 0;
          const prevB = previousData[idx + 2] ?? 0;
          const diff =
